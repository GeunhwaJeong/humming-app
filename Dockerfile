...
# Facade origin baked into the web bundle — a build without this arg targets
# localhost and produces a bundle that cannot reach the real backend.
ARG EXPO_PUBLIC_HUMMING_SERVICE
ENV EXPO_PUBLIC_HUMMING_SERVICE=$EXPO_PUBLIC_HUMMING_SERVICE
# Public web origin of the app (share links, deep-link prefixes). Empty falls
# back to https://humming.social.
ARG EXPO_PUBLIC_HUMMING_APP_URL
ENV EXPO_PUBLIC_HUMMING_APP_URL=$EXPO_PUBLIC_HUMMING_APP_URL
...
RUN echo "Using bundle identifier: $EXPO_PUBLIC_BUNDLE_IDENTIFIER" && \
  ...
  echo "EXPO_PUBLIC_HUMMING_SERVICE=$EXPO_PUBLIC_HUMMING_SERVICE" >> .env && \
  echo "EXPO_PUBLIC_HUMMING_APP_URL=$EXPO_PUBLIC_HUMMING_APP_URL" >> .env && \
